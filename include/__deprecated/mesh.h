...
typedef struct Shader_RTID;
typedef struct Shader_s {
    GLuint vram_id;
} Shader;

ResourceType VertexShader_RTID;
typedef struct VertexShader_s {
    GLuint vram_id;
} VertexShader;
ResourceType FragmentShader_RTID;
typedef struct FragmentShader_s {
    GLuint vram_id;
} FragmentShader;
MeshHandle_load
static void *VertexShader_load(char *path)
{
    char path_buffer[1024];
    resource_file_path(path, ".vert", path_buffer, 1024);
    GLuint vram_id = glCreateShader(GL_VERTEX_SHADER);
    load_and_compile_shader(vram_id, path_buffer);
    void *resource = malloc(sizeof(VertexShader));
    mem_check(resource);
    ((VertexShader *) resource)->vram_id = vram_id;
    return resource;
}
 ...
 */
#define MAX_RENDERER_UNIFORMS 32
ResourceType Renderer_RTID;
typedef struct Renderer_s {
    ...
    GLuint program;
    ResourceHandle shaders[NUM_SHADER_TYPES]; // Shader[]
...
static void *Renderer_load(char *path)
{
...
//================================================================================
void load_mesh_ply(Mesh *mesh, VertexFormat vertex_format, FILE *file, float size_factor);
