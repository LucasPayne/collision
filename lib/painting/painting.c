...
static int g_painting2d_count = 0;
static Paint g_paint[g_painting_size];
static Paint g_paint2d[g_painting_size];
...
typedef struct Canvas_s {
    ...
    Paint paint_buffer[g_painting_size]; // this could later be a dynamic array.
} Canvas;
...
static void painting_add(int canvas, Geometry geometry, ResourceHandle material)
{
    Canvas *canvas = painting_canvas(canvas);
    if (canvas->paint_count >= canvas->paint_buffer_size) {
        fprintf(stderr, ERROR_ALERT "paint error: The maximum number of pieces of paint on a canvas has been exceeded. Possibly the buffer has not been flushed, or there is too much paint.\n");
        exit(EXIT_FAILURE);
    }
    canvas->paint[canvas->paint_count].geometry = geometry;
    canvas->paint[canvas->paint_count].material = material;
    canvas->paint_count ++;
}
void painting_draw(int canvas)
{
    Canvas *canvas = painting_canvas(canvas);
...
void painting_flush(int canvas)
{
    Canvas *canvas = painting_canvas(canvas);
    for (int i = 0; i < canvas->paint_count; i++) {
...
void canvas_paint_line_cv(vec3 a, vec3 b, char *color_str) {
    canvas_paint_line_v(canvas, a, b, str_to_color_key(color_str));
}


// Chain painting (chain of connected line segments).
...
// Loop painting (same as chain except the opposite ends are connected by a line segment).
void canvas_paint_loop(int canvas, float vals[], int num_points, COLOR_SCALARS) // vals length: num_points * 3
{
    ...
    ResourceHandle mat = Material_create("Painting/Materials/flat_color");
    material_set_property_vec4(resource_data(Material, mat), "flat_color", new_vec4(cr,cg,cb,ca));
    canvas_painting_add(canvas, gm_done(), mat);
}
...
void paint_loop_c(int canvas, float vals[], int num_points, char *color_str) {
    canvas_paint_loop_c(canvas, vals, num_points, color_str);
}
void paint2d_loop_c(int canvas, float vals[], int num_points, char *color_str) {
    vec4 color = str_to_color_key(color_str);
    paint2d_loop(canvas, vals, num_points, UNPACK_COLOR(color));
}
